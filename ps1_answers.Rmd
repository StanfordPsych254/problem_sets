...
> ANSWER-START
There is non-uniformity in the Input and Answer formats ("five hundred", an empty string, "5,012"). Additionally, two participants did the task more than once: A1VYX5VKZ0CTWU and A5IUO2T5MZQUZ.
> ANSWER-END
...
```{r}
clean.price <- function(x) {
  gsub("ehight", "8",
       gsub("five hundred", "500",
            gsub(",", "", as.character(x))))
}

d <- transform(d, Answer.dog_cost = clean.price(Answer.dog_cost),
               Answer.plasma_cost = clean.price(Answer.plasma_cost),
               Answer.sushi_cost = clean.price(Answer.sushi_cost))
```
> ANSWER-END
